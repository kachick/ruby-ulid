...
class TestULIDMonotonicGenerator < Test::Unit::TestCase
  ...
  def test_interface
    ...
    assert_equal(ULID.parse(fourth_string), @generator.last)
    fifth = @generator.generate
    assert_instance_of(ULID, fifth)
    assert_equal(fifth, @generator.last)
    assert_equal(
  ...
  def test_generate_and_encode_can_be_used_together
    ...
    assert_equal(Array.new(83) { 1 }, encoded_results.map { |encoded| ULID.parse(encoded) }.each_cons(2).map { |last, succ| succ.to_i - last.to_i })
  end
  ...
  def test_generate_ignores_lower_moment_than_last_is_given
    first = @generator.generate(moment: 42)
  ...
  def test_encode_ignores_lower_moment_than_last_is_given
    first = ULID.parse(@generator.encode(moment: 42))
  ...
  def test_generate_raises_overflow_when_called_on_max_entropy
    ...
    @generator.__send__(:last=, max_ulid_in_a_milliseconds.pred)

    assert_equal(max_ulid_in_a_milliseconds, @generator.generate(moment: max_ulid_in_a_milliseconds.milliseconds))

    @generator.__send__(:last=, max_ulid_in_a_milliseconds)
